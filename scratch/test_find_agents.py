...
from apps.agents.models import Agent, AgentPortfolio, AgentInsuranceSegment
...
def test():
    ...
    user_lat = 23.0222732
    user_lng = 72.5279775
    db_types = []  # No filters initially
    
    # 1. Base Query
    query = Agent.objects.filter(status='active', user__isnull=False)
    query = query.select_related('profile', 'performanceStats', 'user').prefetch_related(
        'insuranceSegments', 'reviews', 'serviceableCities', 'productExpertise'
    )
    print("Base active agents count:", query.count())
    
    # 2. Filter match sql for smart rank
    if db_types:
    ...
    # Force query execution to see if it succeeds
    try:
        all_agents = list(query)
        print("Successfully fetched agents with Smart Rank annotation. Count:", len(all_agents))
    except Exception as e:
        print("ERROR running annotated query:", e)
        return
    ...
    for agent in all_agents:
        ...
        if not agent_coords and agent.profile and agent.profile.city:
            agent_coords = DistanceService.get_city_coordinates(agent.profile.city)
