...
def test():
    ...
    # Test 1: Just RawSQL
    q1 = Agent.objects.filter(status='active', user__isnull=False).annotate(padosi_smart_rank=RawSQL(smart_rank_expr, []))
    print("Test 1 (RawSQL only):", len(list(q1)))

    # Test 2: RawSQL + select_related
    q2 = Agent.objects.filter(status='active', user__isnull=False).select_related('profile', 'performanceStats', 'user').annotate(padosi_smart_rank=RawSQL(smart_rank_expr, []))
    print("Test 2 (RawSQL + select_related):", len(list(q2)))

    # Test 3: RawSQL + prefetch_related
    q3 = Agent.objects.filter(status='active', user__isnull=False).prefetch_related('insuranceSegments', 'reviews', 'serviceableCities', 'productExpertise').annotate(padosi_smart_rank=RawSQL(smart_rank_expr, []))
    print("Test 3 (RawSQL + prefetch_related):", len(list(q3)))

    # Test 4: All combined
    q4 = Agent.objects.filter(status='active', user__isnull=False).select_related('profile', 'performanceStats', 'user').prefetch_related('insuranceSegments', 'reviews', 'serviceableCities', 'productExpertise').annotate(padosi_smart_rank=RawSQL(smart_rank_expr, []))
    print("Test 4 (All combined):", len(list(q4)))
